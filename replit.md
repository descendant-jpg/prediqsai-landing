...
- **Rate limiting**: `middleware/rate-limit.ts` (express-rate-limit). Global: 100 req/15 min per IP on all `/api` routes. AI limiter: 10 req/15 min keyed by userId (fallback IPv6-safe IP) on `POST /chat` and `POST /predictions/refresh`, applied AFTER auth middleware so the user key is set. `sensitiveLimiter` (5 req/15 min) guards the unauthenticated admin password endpoints. All return 429 `{error:"Too many requests"}`. `app.set("trust proxy", 1)` is required for real client IPs behind the Replit proxy. Limits are in-memory (reset on restart, per-instance).
