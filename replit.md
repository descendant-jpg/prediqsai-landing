...
- **Rate limiting**: `middleware/rate-limit.ts` (express-rate-limit). Global: 100 req/15 min per IP on all `/api` routes. AI limiter: 10 req/15 min keyed by userId (fallback IPv6-safe IP) on `POST /chat` and `POST /predictions/refresh`, applied AFTER auth middleware so the user key is set. `sensitiveLimiter` (5 req/15 min) guards the unauthenticated admin password endpoints. Auth routes: `signupLimiter` (5/hour IP) on register, `loginLimiter` (10/15 min IP) on login, and forgot-password stacks `passwordResetIpLimiter` (5/hour IP) + `passwordResetEmailLimiter` (3/hour per normalized email, IP fallback) — these throttle Resend email costs and bcrypt brute-force. All return 429 `{error:"Too many requests"}`. `app.set("trust proxy", 1)` is required for real client IPs behind the Replit proxy. Limits are in-memory (reset on restart, per-instance).
- **Authorization model**: admin status comes ONLY from the shared `requireAdmin` in `middleware/auth.ts` (DB lookup of `users.isAdmin` via `req.userId` from the JWT) — never from request body/query. `PUT /user/me` whitelists username/dailyLossLimit/bankroll; Zod strips injected fields like `isAdmin`/`tier`. `POST /admin/set-admin` is one-time bootstrap: constant-time password check (`timingSafeEqual`) and returns 410 once any admin exists. `POST /admin/verify-password` also uses constant-time comparison.
