import { ArrowLeft, Check, CheckCircle, ChevronRight, Code2, LogOut, Settings, Zap } from "lucide-react-native";
...
const TIERS: { key: Tier; label: string; price: string; color: string; features: string[] }[] = [
  ...
  },
  {
    ...
    price: "$9.99/mo",
    color: "#00E5FF",
    features: ["Unlimited picks", "Full AI analysis & reasoning", "Sharp money signals", "Kelly criterion calculator", "AI assistant (unlimited)"],
  },
  {
    key: "elite",
    label: "Elite",
    price: "$24.99/mo",
    color: "#FFD700",
    features: ["Everything in Pro", "Real-time odds & line movement", "Exclusive high-confidence models", "Performance analytics", "Priority support"],
  },
];
      ...
      >
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
          ...
          <View style={[styles.tierPill, { backgroundColor: `${currentTierData.color}20`, borderColor: `${currentTierData.color}50` }]}>
            <Text style={[styles.tierPillText, { color: currentTierData.color }]}>{currentTierData.label.toUpperCase()}</Text>
          </View>
        </View>
        ...
        <View style={styles.tierGrid}>
          {TIERS.map((tier) => {
            ...
            return (
              <TouchableOpacity
                key={tier.key}
                style={[styles.tierCard, { backgroundColor: isActive ? `${tier.color}12` : colors.card, borderColor: isActive ? tier.color : colors.cardBorder, borderWidth: isActive ? 2 : 1 }]}
        ...
        <View style={[styles.currentTierCard, { backgroundColor: colors.card, borderColor: `${currentTierData.color}30` }]}>
          <Text style={[styles.currentTierTitle, { color: currentTierData.color }]}>{currentTierData.label} — Active Features</Text>
          {currentTierData.features.map((f, i) => (
            <View key={i} style={styles.featureRow}>
              <CheckCircle size={14} color={currentTierData.color} />
...
const styles = StyleSheet.create({
  ...
  section: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14, borderRadius: 16, borderWidth: 1 },
