...
type Post = { title: string; slug: string; excerpt: string | null; content: string; author: string; publishedAt: string };
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return { title: post.title, description, openGraph: { type: "article", title: post.title, description, publishedTime: post.publishedAt, authors: [post.author] } };
  } catch (error) {
...
export default async function BlogArticle({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-night"><article className="mx-auto max-w-3xl px-5 py-14 sm:px-8 sm:py-20">
