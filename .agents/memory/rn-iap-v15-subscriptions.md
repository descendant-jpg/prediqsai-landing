...
**Server-side receipt validation (fail-closed):** the API never trusts client purchase data. `services/iap-validation.ts` verifies iOS receipts via Apple `verifyReceipt` (prod first, 21007 → sandbox retry, needs `APPLE_SHARED_SECRET`) and Android via Play Developer API `purchases.subscriptions.get` (google-auth-library + `GOOGLE_SERVICE_ACCOUNT_JSON`, checks expiry + paymentState 1/2). Verify/restore return 503 when creds are missing and persist only store-confirmed expiry/transaction IDs — never client-supplied ones. Client must send `transactionReceipt` (iOS) / `purchaseToken` (Android).

**Restore ownership transfers:** after Google confirms an active Android entitlement, a restore may move its purchase token from an abandoned account to the account that is restoring it. Perform this as one transaction: clear premium and IAP metadata from any other owner before assigning the unique token and store-confirmed premium metadata to the restoring account.

**Why:** a customer can create a replacement account while their Google Play subscription remains active. Returning a uniqueness conflict blocks a legitimate restore; changing ownership outside a transaction can leave an entitlement without a valid owner.

**How to apply:** this exception is for the restore flow only and only after server-side Google validation confirms the entitlement. Initial purchase verification remains fail-closed against a token already claimed by another account.
