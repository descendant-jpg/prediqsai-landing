...
- **A screen that hosts the language picker may serve stale text on native even when context is correct** — react-navigation can reuse a memoized scene element, so the screen the switch happens on lags while sibling tabs refresh on focus. Durable remedy: force-remount that host screen's visible subtree on locale change (key its root container by `locale`). The context wiring is not the defect; this is a deterministic guard for the host screen. If it persists, suspect a stale device bundle or that the stale element is the tab title/options, not the body.
